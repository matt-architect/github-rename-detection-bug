    ...
    public class OrderHeaderOrderInfoProfile : Profile
    {
        public OrderHeaderOrderInfoProfile()
        {
            ...
            CreateMap<OrderHeaderOrderInfoEntity, OrderHeaderOrderInfoResponse>()
                .ForMember(dest => dest.AllergySummary,
                    opt => opt.MapFrom(src => MapAllergySummary(src)));
            CreateMap<AllergyDetail, AllergyItem>();
        }
        private static AllergySummary MapAllergySummary(OrderHeaderOrderInfoEntity entity)
        {
            if (entity?.Allergies == null)
            {
                ...
                {
                    AllergyCount = 0,
                    Allergies = new System.Collections.Generic.List<AllergyItem>()
                };
            }
